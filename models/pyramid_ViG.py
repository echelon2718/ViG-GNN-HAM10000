...
class Stem(nn.Module):
    def __init__(self, in_dim=3, out_dim=768, act='relu'):
        super().__init__()
        self.convs = self.create_convolutional_layers(in_dim, out_dim, act)
    
    def forward(self, x):
        out = self.convs(x)
        return out
    
    def create_convolutional_layers(self, in_dim, out_dim, act):
        return Sequential(
...
class DeepGCN(nn.Module):
    def __init__(self, opt):
        ...
        self.pos_embedding = nn.Parameter(torch.zeros(1, channels[0], img_size // 4, img_size // 4))
        ...
        for i in range(len(blocks)):
            ...
            for _ in range(blocks[i]):
                self.backbone += [
                    Sequential(Grapher(channels[i], 
                                       num_knn[idx], 
                                       min(idx // 4 + 1, max_dilation), 
                                       conv, act, norm,
                                       bias, stochastic, epsilon,
                                       reduce_ratios[i], n=HW,
                                       drop_path=dpr[idx],
                                       relative_pos=True),
        ...
        self.prediction = Sequential(nn.Conv2d(channels[-1], 1024, 1, bias=True),
                                     nn.BatchNorm2d(1024),
                                    act_layer(act),
    ...
    def forward(self, inputs):
        x = self.stem(inputs) + self.pos_embedding
...
@register_model
def PyramidVIG_Tiny_GELU(pretrained=False, **kwargs):
    class OptInit:
    ...
    opt = OptInit(**kwargs)
    model = DeepGCN(opt)
    model.default_cfg = default_cfgs['vig_256_gelu']
    return model

@register_model
def PyramidVIG_Small_GELU(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=7, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
...
@register_model
def PyramidVIG_Medium_GELU(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=7, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
...
@register_model
def PyramidVIG_Large_GELU(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=7, drop_path_rate=0.0, **kwargs):
            self.k = 9 # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
    ...
    opt = OptInit(**kwargs)
    model = DeepGCN(opt)
    model.default_cfg = default_cfgs['vig_b_256_gelu']
    return model