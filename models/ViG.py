...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 7, 'input_size': (3, 256, 256), 'pool_size': None,
...
@register_model
def VIG_Tiny_GELU(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=7, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
    ...
    opt = OptInit(**kwargs)
    model = DeepGCN(opt)
    model.default_cfg = default_cfgs['gnn_patch16_256']
    return model


@register_model
def VIG_Small_GELU(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=7, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            self.k = num_knn # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
...
@register_model
def VIG_Big_GELU(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=7, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            self.k = num_knn # neighbor num (default:9)
            self.conv = 'mr' # graph conv layer {edge, mr}
